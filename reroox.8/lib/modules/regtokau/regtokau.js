...
module.exports = function() {
    ...
    router.post('/auth', function(req, res) {
        ...
        const appid = ja[1]
        const coid = ja[0]
        var query1 = conn.query('SELECT * FROM whoapp  WHERE emailid = ? AND appid = ? AND coid= ?', [payload.email, appid, coid], function(error, results) {
                cons.log(query1.sql)
                if (results.length == 0) {
                    ...
                    var query = conn.query("UPDATE whoapp SET auth= true WHERE emailid = ? AND appid = ? AND coid = ?", [payload.email, appid, coid], function(error, results) {
                        cons.log(query.sql)
                        cons.log(error)
                        if (error) {
