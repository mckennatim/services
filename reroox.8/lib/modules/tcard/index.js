...
module.exports = function() {
    ...
    router.put('/updjc', bearerTokenApp, function(req, res) {
        if (!req.userTok.auth) {
            ...
            let jcdata = { wdprt: wdprt, emailid: req.userTok.emailId, coid: req.userTok.coId }
            var query = conn.query('DELETE FROM tcardjc WHERE wdprt=? AND emailid=? AND coid=? ', [wdprt, req.userTok.emailId, req.userTok.coId], function(error) {
                cons.log(query.sql)
                cons.log(error)
                jcost.map((jc) => {
                    jcdata.job = jc.job
    ...
    router.get('/bweek/:wk', bearerTokenApp, function(req, res) {
        if (!req.userTok.auth) {
            ...
            const wdprt = `${yr}-W${wk.toString().padStart(2,'0')}%`
            cons.log(wdprt)
            var query = conn.query('SELECT `wdprt`, `inout`, `hrs` FROM tcardpu WHERE emailid = ? AND coid = ? AND wdprt LIKE(?)', [req.userTok.emailId, req.userTok.coId, wdprt], function(error, punch) {
                cons.log(query.sql)
                cons.log(error)
    ...
    router.get('/week/:wk', bearerTokenApp, function(req, res) {
        if (!req.userTok.auth) {
            ...
            const wprt = `${yr}-W${wk.toString().padStart(2,'0')}`
            const wdprt = `${wprt}%`
            var query0 = conn.query('SELECT `wprt`, `status`, `hrs` FROM tcardwk WHERE emailid = ? AND coid = ? AND wprt =?', [req.userTok.emailId, req.userTok.coId, wprt], function(error0, wstat) {
                cons.log(query0.sql)
                cons.log(error0)
                cons.log(wstat)
                cons.log(wdprt)
    ...
    router.delete('/del', bearerTokenApp, function(req, res) {
        if (!req.userTok.auth) {
            ...
            var query1 = conn.query('DELETE FROM tcardpu WHERE wdprt=? AND emailid=? AND coid=?', [wdprt, req.userTok.emailId, req.userTok.coId], function(error, results) {
                cons.log(query1.sql)
                cons.log(error)
                cons.log(results)
                res.jsonp(results)
