...
module.exports = function() {
	...
	router.put('/test2', function(req, res) {
		...
		console.log(keys);
		console.log(vals);
		var query1 = conn.query('DELETE FROM jobcatact WHERE job=? AND coid=? AND week=0', [job, coid], function(error,results,fields){
			cons.log(query1.sql)
			cons.log(error)
			cons.log(results)
	...
	router.get('/list/:wk', bearerTokenApp, function(req,res){
		if(!req.userTok.auth){
			...
			cons.log(req.params)
			var q =conn.query('SELECT * FROM jobcatact WHERE week=? AND coid=? ORDER BY idx, category', [req.params.wk*1, req.userTok.coId ], function(error, results, fields){
				cons.log(q.sql)
	...
	router.post('/post/:wk', bearerTokenApp, function(req,res){
		if(!req.userTok.auth){
			...
			console.log(keys);
			console.log(vals);
			var query1 = conn.query('DELETE FROM jobcatact WHERE week=? AND coid=?', [week, req.userTok.coId], function(error,results,fields){
				cons.log(query1.sql)
				cons.log(error)
				cons.log(results)
				var query = conn.query('INSERT INTO jobcatact ('+keys+') VALUES ? ', vals, function(error,results,fields){
